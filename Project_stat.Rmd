...
The following plot points out the difference between the two kind of hotels, since most of the reservations related to the City Hotel last approximately 2-3 nights, while in case of the Resort Hotel (come prima) the same pattern is still observed, but in the meantime also 7 nights stand out as being a very popular choice among guests. More in general a long stay is very unusual in case of City Hotels.
...
In the plot of 2016 (avevamo detto di non sottolineare Giugno ma di parlare dell’andamento del periodo estivo) a strange behavior is displayed and we can detect a drop in the number of bookings during summertime. Therefore we decided to make a plot with the number of guests for each month and the result is a graph where this value seems to be higher during summertime, which is what we expected.
...
```{r warning=FALSE}
# Boxplot to show cancelleations based on lead_time
...
ggplot(hotel_bookings_new, aes(x = lead_time, fill = is_canceled)) + 
  ...
  labs(title = "Average Daily rate by Hotel",
       x = "Lead Time",
       y = "Count") + scale_fill_manual(values=c("#EF7365", "#FBD26C"),
    name = "Booking Status",
    breaks = c("0", "1"),
    labels = c("Cancelled", "Not Cancelled")) + 
  xlim(0,500) +
...
fig_canc <- df_canc %>% plot_ly(labels = ~is_canceled, values = ~frequency, 
                                  marker = list(colors = colors_donut,
                                  line = list(color = '#FFFFFF', width = 1)))
...
The remainder of this analysis  will be focused on *is_canceled*, which is a binary variable (with values: 0,1). The aim is to find from the observations if a booking will be canceled or not. To achieve this purpose we need to take into account that the most of the variables are categorical variables. It’s very common to start this kind of analysis by verifying the correlation between variables, so to understand what is their relationship. The presence of a so great amount of categorical variables in the case under examination encouraged us to use a Chi-squared test to see the relationship between the response variable *is_canceled* and other variables.
Most of these informations come directly from the Exploratory Data Analysis, where we have assessed what is the relationship between *is_canceled* and some specific variables that will be of interest in the continuation of the analysis. For example, the variable *distribution_channel* has been shown to be strictly related to *market_segment* (nope, non lo mostriamo, lo notiamo direttamente dal dataset), since they have almost the same levels. Regarding the variable *deposite_type*, we saw that the impact on the cancellation was very high, also in a counterintuitive way (in the already discussed non refundable case). We want now to verify all these intuitions given by the previous exploratory analysis with the Chi-squared test.
We start from the couple *is_canceled*/*market_segment*:

```{r warning=FALSE}

attach(hotel_bookings_new)
ct_ms <-table(is_canceled, market_segment)
kable(ct_ms)
```

```{r warning=FALSE}
test_ms <-chisq.test(ct_ms)
corrplot(test_ms$residuals, is.cor = FALSE)
```
...
attach(hotel_bookings_new)
ct_dt <-table(is_canceled, deposit_type)
kable(ct_dt)
```

```{r warning=FALSE}
test_dt <-chisq.test(ct_dt)
corrplot(test_dt$residuals, is.cor = FALSE)
```


The analysis seems to be a confirmation of what we have understood from the exploratory analysis.

Another thing to be noticed is that *reservation_status* is clearly a redundant variable, since it is exactly the same as *is_canceled*.
...
ct<-table(is_canceled, arrival_date_month)
kable(ct)
```
